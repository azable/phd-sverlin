...
- `src/lib/projects/` is the browser-safe project feature boundary. `events/` defines the immutable event contracts and exhaustive dispatch API, `model.ts` defines documents and transport models, and `projection.ts` reconstructs historical state. The Svelte components and `project-session.svelte.ts` own browser state and UI only.
- `src/lib/timeline/` renders every project event and owns feedback composition, including focused history and compact visual selections.
...
- `src/lib/server/projects/` is the corresponding SvelteKit-enforced server-only boundary. It owns project persistence, whole-command serialization, AI orchestration, prompt projection, artifact blobs, and compilation event recording; browser modules must never import it.
- `src/lib/server/compile-visualization.ts` runs the Haskell compiler against an isolated source snapshot.
...
Project state is retained under the ignored `data/projects/<project-id>/` directory by default; set `SVERLIN_PROJECT_DIR` to move the root. `project.json` is the readable v1 event log. Each event's numeric `id` is its stable 1-based array position; a UUID `operationId` groups the visible stages of one command. Source versions, successful render JSON, compiler stdout/stderr, AI prompts, provider responses, candidate source, and provider error details are content-addressed blobs under the same project directory. Writes are atomic, blobs are integrity-checked when read, and there is no automatic retention limit. Project files can contain complete source and user requests and are never uploaded automatically except as context for an explicit AI feedback request.
...
The workspace has a resizable Timeline beside the visualization and source artifact. Every internal operation is visible as its own event; there is no separate chat history, edit history, or compilation-error store. A per-project server-sent event feed publishes events only after their atomic project write, so AI, compilation, repair, and activation stages appear while a command is still running. Reconnection catches up from a persisted numeric event ID without making the stream a second source of truth. Select an old event to reconstruct that exact read-only project state; its visualization can still be stepped, selected, panned, and zoomed. **Restore** copies its artifacts forward as new events rather than changing history. Prior events can be focused while browsing history and then included in feedback after returning to the present. Canvas feedback stores only the render event, checkpoint, selected render-instance IDs, and judgement; the server resolves their semantic roles and solved styles from the immutable render blob when building AI context.

Editing source, changing a seed, restoring, and giving AI feedback are project commands. The interface stays locked while their correlated Timeline events arrive and unlocks only after the command response atomically installs the authoritative project view—including a newly loaded visualization when compilation succeeds. The live delivery adapter is process-local, matching the single Node process and local project directory used by the app; durable catch-up survives browser reconnections and server restarts. Manual source is retained even when it fails compilation, with no stale visualization paired to it. AI source remains an unaccepted candidate until it compiles. A source or pipeline failure permits exactly one explicit repair generation and compile; provider retries are disabled, infrastructure failures are not sent back for repair, and there is no internal retry loop. A failed attempt and its terminal notice remain immutable Timeline events.
...
The schema deliberately remains version 1 while the product is refined. Valibot schemas are the source of the TypeScript event and command types, and event IDs must be contiguous from 1. Historical state is derived by a pure fold over the prefix ending at the requested event. Every event must be added to the aggregate event schema and exhaustively classified by the state projection, Timeline presentation, AI prompt Timeline, and AI conversation projection; TypeScript reports a missing case. The state transition registry also determines whether a streamed event requires the browser to reload hydrated state. Concurrent mutations include the expected numeric head so a stale command cannot overwrite a newer result. The client loads a complete projection from `GET /api/projects/:projectId?at=<event-id>`, submits discriminated JSON commands to `POST /api/projects/:projectId` and receives the resulting hydrated view, and follows durable events from `GET /api/projects/:projectId/events?after=<event-id>`.
...
The chatbot receives the current artifacts, the complete semantic Timeline, conversation messages derived from feedback and assistant-response events, and fully resolved artifacts for explicit historical references. It never receives an arbitrary “last N” window. Exact provider prompts and responses are stored for later prompt evaluation but prompt bodies are referenced by hashes rather than recursively embedded in future contexts. `CHATBOT_MAX_CONTEXT_CHARS` (default `500000`) is only a guard against exceeding the provider context; it fails explicitly rather than dropping history. The source editor uses a small Svelte 5 runes adapter over modular CodeMirror 6.
