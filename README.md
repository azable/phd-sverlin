...
`sverlin` is a project-based SvelteKit trace viewer backed by a Haskell trace compiler and visualization solver.
...
- `src/lib/projects/` contains the strongly typed v1 project event model, runtime validator, pure historical reducer, and Svelte 5 project session.
...
- `src/lib/server/projects/` owns project persistence, whole-command serialization, AI orchestration, prompt projection, artifact blobs, and compilation event recording.
...
The schema deliberately remains version 1 while the product is refined. Valibot schemas are the source of the TypeScript event and command types, and event IDs must be contiguous from 1. Historical state is derived by a pure fold over the prefix ending at the requested event. Concurrent mutations include the expected numeric head so a stale command cannot overwrite a newer result. The client loads a complete projection from `GET /api/projects/:projectId?at=<event-id>`, submits discriminated JSON commands to `POST /api/projects/:projectId`, and follows durable events from `GET /api/projects/:projectId/events?after=<event-id>`.
