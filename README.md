...
- `compile/src/LinearTrace/` contains the reusable trace model, public choreography DSL, internal view graph compiler, and target pipeline. `Choreography` owns the runtime bridge between Core trace events and View render output; `View` owns neutral view ids, labels, style/layout primitives, render intents, and graph solving; `Compile` lowers the solved graph directly into the canonical IR.
- `compile/src/LinearTrace/Visualization/IR.hs` is the canonical renderer-neutral visualization intermediate representation. It mirrors the visual DSL with concrete solved style values, field-level CSP variable references, an immutable element registry, and complete scene snapshots. Future artifact renderers compile this solved IR through `LinearTrace.Visualization.Target` and must not make additional seeded choices.
