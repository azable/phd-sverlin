...
- Body-only Sverlin compilation enables balanced automatic style profiles after authored rules and selected-style access have been applied. Trace leaves share a profile and palette by payload type, generated containers share by group identity, and `styleFamily` can override that inference. Direct Haskell use of `runChoreographyWith` retains sparse omission semantics.
- `compile/src/LinearTrace/Visualization/IR.hs` is the canonical renderer-neutral visualization intermediate representation. Version 2 contains concrete solved style values, explicit coordinates and text baselines, exact resource descriptors, compiler findings, sparse field-to-CSP-variable traceback, an immutable element registry, and named timeline steps. Each checkpoint introduces exactly one step containing its visible instances. Artifact renderers consume a versioned `CompilationPackage` through the pure `LinearTrace.Visualization.Target` boundary and must not make additional seeded choices.
...
- `cabal.project` anchors the Haskell project at the repository root. Project scripts pass `--builddir=compile/dist-newstyle` so their Cabal build artifacts stay under `compile/`; use the same flag for ad hoc Cabal commands because Cabal does not support a project-file build-directory default.
...
Text is shaped with HarfBuzz between the initial aesthetic solve and a second constraint solve. The second solve pins every finite aesthetic choice, reuses the first solution as its initialization, and adds hard content-box width and exact vertical ink/line-box constraints, so typography cannot silently change the seed's design family. Compiler-selected lines and baselines are authoritative; the SVG browser renderer loads the exact project font with `FontFace`, emits one SVG text node per line, and never delegates wrapping to CSS. Text stays hidden if the exact font is unavailable instead of falling back silently. Runtime metric mismatch observations remain ephemeral and separate from immutable compiler findings.

Automatic profile text may retain its preferred size or shrink to 12 px in 0.25 px steps. A literal `style @FontSize` stays fixed; use `fitText "label"` instead of `content "label"` to opt that explicit preferred size into fitting. Normal text prefers one line and may insert at most two deterministic breaks. `WhiteSpacePre` preserves authored lines, while compiler no-fit and missing-glyph cases fail with actionable diagnostics. Size reduction, minimum physical size, inserted wrapping, and static weight substitution remain inspectable as IR findings and are included in AI project context.
...
The visualization regeneration path uses a view-specific penalty configuration only when optimizer fallback is selected, with a stricter retry if that solve fails the success/energy check. Affine samples do not run that redundant retry. Shrink-wrapped groups no longer build nonlinear nested `minExpr`/`maxExpr` trees: containment remains affine and four named child-edge decisions express which child is tight. This removes the production reason group layout previously selected the penalty optimizer. The legacy optimizer still shares nested minimum, maximum, and squared energy operands within an evaluation for explicit nonlinear problems and the stable `nested-extrema` fixture. Compile commands with `--details` print the selected solver backend and its work counters alongside phase timings for dynamic source loading, view graph construction, solving, IR compilation, target encoding, and writing.
...
The devcontainer post-create step updates Cabal's package index and runs `cabal build -v0 compile-app --builddir=compile/dist-newstyle` to warm the build artifacts before the first browser-triggered regeneration; it does not need to produce visualization JSON. A minimal runtime Cabal config keeps repository policy explicit while allowing Cabal's XDG defaults to select its mutable paths. Cache and state live in persistent named volumes mounted at the workspace-local `.cache/cabal` and `.local/state/cabal`; HLS also uses the workspace-local XDG cache. Those paths are writable to workspace-sandboxed tools, survive container rebuilds, and keep package indexes, build summaries, and the shared store out of `/root`. Web regeneration has a server-side timeout controlled by `SVERLIN_COMPILE_TIMEOUT_MS`; it defaults to `300000` milliseconds, and the devcontainer sets that value explicitly.
...
- Choreography style assignment uses `render selected $ do ...` for node patches. `content` uses managed plain typography, while `fitText` opts an explicit preferred size into bounded fitting. Verbatim code composes as `highlightCode "haskell" (codeWrap (codeContent "..."))`; omit either wrapper when highlighting or inserted code wrapping is not wanted, and use `emphasizeCode "step" [codeRange start end]` for checkpoint-specific source emphasis. `style @Field ...` requires a field, `withoutStyle @Field` forbids it, omission delegates it to the body-only runner's automatic family profile, and `styleCase @Field choice` exhaustively controls conditional presence. `styleFamily "key"` overrides payload/group family inference. Selected style constraints use `styleOf @Field selected` and therefore require guaranteed presence. Center positioning uses `center selected` in constraints and `center (vec2 x y)` inside `render`. Numeric variables use `variable @Span`/`variable @Angle`; categorical variables use `choice @FontFamily`. Substantially different numeric compositions use `oneOf "name" (alternative "first" [...]) [alternative "second" [...]]`; `caseOf` exhaustively connects an existing typed choice to numeric visual constraints.
