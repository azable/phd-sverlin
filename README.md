...
- `src/lib/timeline/` renders every project event and owns feedback composition, including explicit historical and visual-selection attachments.
...
The workspace has a resizable Timeline beside the visualization and source artifact. Every internal operation is visible as its own event; there is no separate chat history, edit history, or compilation-error store. A per-project server-sent event feed publishes events only after their atomic project write, so AI, compilation, repair, and activation stages appear while a command is still running. Reconnection catches up from the persisted event sequence without making the stream a second source of truth. Select an old event to reconstruct that exact read-only project state. **Restore** copies its artifacts forward as new events rather than changing history. Old events can be attached explicitly to present feedback. Canvas selections capture the render, checkpoint, instance identities, semantic roles, solved styles, and whether the user considers them a reference, preferred, or undesired.
...
- rendering and compilation: `visualization.render-requested`, `compilation.requested`, `compilation.succeeded`, `compilation.failed`, `visualization.rendered`.

The schema deliberately remains version 1 while the product is refined. Known envelopes and payloads are runtime-validated, event positions must form one contiguous parent chain, and additive fields are retained for experimentation. Historical state is derived by a pure fold over the prefix ending at the requested event. Concurrent mutations use an expected head event ID so a stale command cannot overwrite a newer result.
