...
- L-BFGS-B available as `liblbfgsb` for nonlinear fallback, BLAS/LAPACK for `hmatrix` affine reduction, and the `highs` executable for MIP-conditioned disjunctions above the exact-enumeration limit. The devcontainer installs the native libraries and builds a pinned HiGHS release.
