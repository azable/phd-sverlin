...
- To run the Haskell application manually with a seed-based workspace output path, use `pnpm run compile -- --seed 1` from the root directory. Pass `--output FILE` for an explicit path or when omitting `--seed`; the web app no longer reads `static/compiled.json`.
...
- `pnpm run compile -- --seed 1 --details` includes phase timings for the view graph, solver, materialization, JSON encoding, and JSON writing. Seeded manual, compile server, and benchmark paths use generated JSON paths grouped under the ignored workspace `outputs/seed-<seed>/` directory by default; stdout/stderr are diagnostic logs, including when `--json --details` is passed.
...
- If modifying the Haskell application, run `pnpm run compile -- --seed 1` from the root directory to compile and run the Haskell application.
