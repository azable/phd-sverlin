...
- `./compile.sh --details` includes phase timings for the view graph, solver, materialization, JSON encoding, and JSON writing. The compile server and benchmark paths use a file output path for generated JSON; stdout/stderr are diagnostic logs, including when `--json --details` is passed.
