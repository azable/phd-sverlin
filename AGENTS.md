...
- `./compile.sh --details` includes phase timings for the view graph, solver, materialization, JSON encoding, and JSON writing. Keep diagnostics on stderr when `--json --details` is used so stdout remains valid JSON.
